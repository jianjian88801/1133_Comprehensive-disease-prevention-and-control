    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="churujiluTypes">
                        <el-select v-model="ruleForm.churujiluTypes" :disabled="ro.churujiluTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in churujiluTypesOptions"
                    ...
                    <div v-else>
                        <el-form-item class="input" label="类型" prop="churujiluValue">
                        <el-input v-model="ruleForm.churujiluValue"
                            placeholder="类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item class="upload" v-if="type!='info' && !ro.churujiluPhoto" label="健康码" prop="churujiluPhoto">
                        <file-upload
                            tip="点击上传健康码"
                            action="file/upload"
                            :limit="3"
                            :multiple="true"
                            :fileUrls="ruleForm.churujiluPhoto?ruleForm.churujiluPhoto:''"
                            @change="churujiluPhotoUploadChange"
                        ></file-upload>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.churujiluPhoto" label="健康码" prop="churujiluPhoto">
                            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in (ruleForm.churujiluPhoto || '').split(',')" :src="item" width="100" height="100">
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="备注" prop="churujiluContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.churujiluContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.churujiluContent" label="备注" prop="churujiluContent">
                            <span v-html="ruleForm.churujiluContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    yonghuId: false,
                    churujiluTypes: false,
                    churujiluPhoto: false,
                    churujiluContent: false,
                    churuTime: false,
                    insertTime: false,
                },
                ruleForm: {
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=churujilu_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.churujiluTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`churujilu/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.churujiluCrossAddOrUpdateFlag = false;
                                        this.parent.search();
            ...
            //图片
            churujiluPhotoUploadChange(fileUrls){
                this.ruleForm.churujiluPhoto = fileUrls;
                this.addEditUploadStyleChange()
            },
