    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  class="input" label="检测时间" prop="jianceTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.jianceTime"
                                type="datetime"
                                placeholder="检测时间"
                                :disabled="ro.jianceTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.jianceTime" label="检测时间" prop="jianceTime">
                            <span v-html="ruleForm.jianceTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="检测结果" prop="jianceTypes">
                        <el-select v-model="ruleForm.jianceTypes" :disabled="ro.jianceTypes" placeholder="请选择检测结果">
                            <el-option
                                v-for="(item,index) in jianceTypesOptions"
                    ...
                    <div v-else>
                        <el-form-item class="input" label="检测结果" prop="jianceValue">
                        <el-input v-model="ruleForm.jianceValue"
                            placeholder="检测结果" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="备注" prop="hesuanjianceContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.hesuanjianceContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.hesuanjianceContent" label="备注" prop="hesuanjianceContent">
                            <span v-html="ruleForm.hesuanjianceContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=jiance_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.jianceTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`hesuanjiance/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.hesuanjianceCrossAddOrUpdateFlag = false;
                                        this.parent.search();
