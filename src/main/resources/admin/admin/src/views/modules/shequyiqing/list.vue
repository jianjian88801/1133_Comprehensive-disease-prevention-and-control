        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.shequyiqingName" placeholder="标题" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shequyiqing','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jibingfangkong/upload/shequyiqingMuBan.xls"
                        >批量导入社区疫情情况数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shequyiqing','导入导出')"
                                ...
                                :on-success="shequyiqingUploadSuccess"
                                :on-error="shequyiqingUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shequyiqing','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入社区疫情情况数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shequyiqing','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shequyiqing.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shequyiqingName"
                                   header-align="center"
                                   label="标题">
                        <template slot-scope="scope">
                            {{scope.row.shequyiqingName}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('shequyiqing','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shequyiqing','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shequyiqing','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                yonghuId : null,
                shequyiqingName : null,
                gonggaoPhoto : null,
                shequyiqingContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"shequyiqing",//当前表表名,
                        sumColum : 'shequyiqing_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shequyiqing_name',//分组字符串字段
                        // types : 'shequyiqing_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.shequyiqingName!= '' && this.searchForm.shequyiqingName!= undefined) {
                    params['shequyiqingName'] = '%' + this.searchForm.shequyiqingName + '%'
                }
                                
                params['shequyiqingDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "shequyiqing/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            shequyiqingUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shequyiqing/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入社区疫情情况数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
