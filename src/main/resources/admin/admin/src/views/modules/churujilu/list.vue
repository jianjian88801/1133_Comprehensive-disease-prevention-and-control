                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('churujilu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jibingfangkong/upload/churujiluMuBan.xls"
                        >批量导入出入记录数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('churujilu','导入导出')"
                                ...
                                :on-success="churujiluUploadSuccess"
                                :on-error="churujiluUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('churujilu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入出入记录数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('churujilu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "churujilu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.churujiluValue}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="churujiluPhoto"
                                     header-align="center"
                                     width="200"
                                     label="健康码">
                        <template slot-scope="scope">
                            <div v-if="scope.row.churujiluPhoto">
                                <img :src="scope.row.churujiluPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('churujilu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('churujilu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('churujilu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :"churujilu",//当前表表名,
                        sumColum : 'churujilu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'churujilu_name',//分组字符串字段
                        // types : 'churujilu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['churujiluDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            churujiluUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "churujilu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
